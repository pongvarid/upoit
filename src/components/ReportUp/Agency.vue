...
export default class TestDevClass extends Vue {
  ...
  async loadEnv(){
    ...
    this.result.AA = await _.filter(this.lists ,{rate:'AA'})
    this.result.A = await _.orderBy(_.filter(this.lists ,{rate:'A'}),(r:any)=>{ return -r.all} )
    this.result.B = await _.orderBy(_.filter(this.lists ,{rate:'B'}),(r:any)=>{ return -r.all} )
    this.result.C = await _.orderBy(_.filter(this.lists ,{rate:'C'}),(r:any)=>{ return -r.all} )
    this.result.D = await _.orderBy(_.filter(this.lists ,{rate:'D'}),(r:any)=>{ return -r.all} )
    this.result.E = await _.orderBy(_.filter(this.lists ,{rate:'E'}),(r:any)=>{ return -r.all} )
    this.result.F = await _.orderBy(_.filter(this.lists ,{rate:'F'}),(r:any)=>{ return -r.all} )
    this.result.Other = await _.orderBy(_.filter(this.lists ,(r:any)=>{ return ( r.rate != 'AA' && r.rate != 'A' && r.rate != 'B' && r.rate != 'C' && r.rate != 'D' && r.rate != 'E'&& r.rate != 'F')} ),(r:any)=>{ return -r.all} )
    this.response = true;
  }
