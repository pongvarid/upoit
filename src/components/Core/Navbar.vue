  ...
  <div>
    ...
    <v-navigation-drawer v-model="drawer" app    fixed class="bg-list" > <br>
      <center> <div class="bg-white shadow-xl rounded-full w-4 h-4 md:w-32 md:h-32 p-2 border-8 border-yellow-600 flex justify-center items-center">
        ...
        <h2 class="text-sm md:text-2xl text-purple-800  cv-font">UP-ITA</h2>  <p class="bg-yellow-700 ml-4 mr-4 p-1 text-white rounded-full">เปิดประตูสู่ความโปร่งใส</p></center>
      <v-list nav  dense >
    ...
    <br>
        ...
        <v-list-item-group ><br>
          <v-list-item @click="$router.push('/alert')" :class="($route.name == 'all-alert' )?'bg-purple-600':''">
            <v-list-item-title><v-icon style="font-size:18px;">em em-zap</v-icon>  <span class="text-base" :class="($route.name == 'all-alert' )?'text-white':''">แจ้งปัญหาการใช้งาน</span></v-list-item-title>
          </v-list-item>
          <v-list-item @click="logout()" >
            <v-list-item-title><v-icon style="font-size:18px;">em em-arrow_right_hook</v-icon>  <span class="text-base">ออกจากระบบ</span></v-list-item-title>
          </v-list-item>
        </v-list-item-group>
      </v-list>
    </v-navigation-drawer>
