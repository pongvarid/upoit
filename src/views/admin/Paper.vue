            ...
            <div class="relative w-full mt-4 mb-4 max-w-full flex-grow flex-1 px-2 py-2" v-if="response">
                <div class="flex flex-wrap">
                    ...
                    <div class="block w-full overflow-x-auto bg-gray-100">
                        ...
                        <v-expansion-panels>
                            <v-expansion-panel v-for="(oit,index) in rates" :key="index" v-if="checkDataTest((oit.evaluate.score || oit.evaluate.score == 0)) && checkNumData(oit.number)">
                                <v-expansion-panel-header>

                                    <h2 class="text-base font-bold" style="width:450px;">

                                        <span class="text-purple-600 font-bold">O-{{oit.number}} &nbsp;&nbsp;</span>&nbsp;&nbsp;{{oit.name}} ({{ oit.evaluate.score }})

                                    </h2>

                                    <v-chip v-if="passingAllCheckTrue(oit.result)" class="mr-4" color="primary" small style="width:50px!important;">
                                        <v-icon left> mdi-check-network</v-icon> หัวหน้าหน่วยงานยืนยันแล้ว
                                    </v-chip>
                                    <v-chip v-else class="mr-4" small style="width:50px!important;">
                                        <v-icon left> mdi-close-network </v-icon> หัวหน้าหน่วยงานไม่ได้ยืนยัน
                                    </v-chip>

                                    <v-chip small style="width:10px!important;" v-if="oit.evaluate.rate_status != 1 && (oit.evaluate.score || oit.evaluate.score == 0)" small color="success" text-color="white">
                                        <v-icon left> mdi-marker-check </v-icon> ตรวจแล้ว
                                    </v-chip>
                                    <v-chip v-else-if="oit.evaluate.rate_status == 1" class="mr-4" color="warning" small style="width:50px!important;">
                                        <v-icon left> mdi-check-network</v-icon> รอตรวจสอบผลการตรวจ
                                    </v-chip>
                                    <v-chip small style="width:10px!important;" v-else small>
                                        <v-icon left>mdi-do-not-disturb-off</v-icon> ยังไม่ได้ตรวจ
                                    </v-chip>
                                    ...
                                    <div class="flex flex-col md:flex-row">
                                        ...
                                        <div class="w-full md:w-1/3">
                                            <form v-if="passingAllCheckTrue(oit.result)" @submit.prevent="(oit.evaluate.id)?updateEvaluate(oit):storeEvaluate(oit)" class="shadow-2xl md:border-t-2 md:border-b-2 md:border-purple-500 rounded-xl md:m-2 md:p-6 p-2">
                                                ...
                                                <v-select required v-model="oit.evaluate.rate_status" label="สถานะการตรวจสอบ" placeholder="ระบุสถานะ" :items="rateStatus" item-text="name" item-value="id" filled></v-select>
                                                ...
                                                <br>
                                                <center v-if="(oit.evaluate.id)" class="mt-6 mb-4">
                                                    <v-btn type="submit" color="orange" dark large>แก้ไขการให้คะแนน</v-btn>
                                                </center>
                                                <center v-else class="mt-6 mb-4">
                                                    <v-btn type="submit" color="success" dark large>บันทึกการให้คะแนน</v-btn>
                                                </center>
                                            </form>
                                        </div>
                                    </div>
