            ...
            <div class="flex flex-wrap -mx-4">
              <div class="w-full lg:w-1/4 px-4 mb-4 lg:mb-0">
                <div class="h-35 rounded-xl shadow-xl bg-white border-b-4 border-yellow-600">
                  <div class="flex items-center justify-between px-4 py-3 border-b bg-yellow-600 rounded-t-xl">
                    <h3 class="text-lg font-semibold font-heading text-white">บุคคลากรภายนอก</h3>
                  </div>
              ...
              <div class="w-full lg:w-1/4 px-4 mb-4 lg:mb-0">
                <div class="h-35 rounded-xl shadow-xl bg-white border-b-4 border-green-600">
                  ...
                  <div class="flex flex-col p-4">
                    <h3 class="text-xl mb-3 font-semibold font-heading font-semibold"> {{score30}} <span class="text-sm">(คิด 40%)</span></h3>
...
export default class Home extends Vue {
  ...
  private async run() {
    ...
    this.agency = await Core.getHttp(`/api/ita/v1/agency/${21}/`)
    this.yearData = await Core.getHttp(`/api/eit/v2/year/${2}/`)
  ...
  private async getRawIssue(assignId: number) {
    this.issueRaw = await Core.getHttp(`/api/eit/v2/answerissueeit-report/?agency=${21}&assessmentIssues=${assignId}`)
  }
  ...
  async getAverage(){
    ...
    for (let i=0; i < this.assessmentData.length ; i++){
      let raw = await Core.getHttp(`/api/eit/v2/answerissueeit-report/?agency=${21}&assessmentIssues=${this.assessmentData[i].id}`)
