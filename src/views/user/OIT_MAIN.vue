    ...
    <div class="relative w-full mt-4 mb-4 max-w-full flex-grow flex-1 px-2 py-2" v-if="response && user.ext_link.oit">
        <div class="grid gap-6 mb-8 md:grid-cols-2 xl:grid-cols-3">
            <div class="w-full   " v-for="year,i in years" :key="i">
              <div @click="$router.push(`/user/paper?year=${year.id}`)"  class="flex cursor-pointer	 items-center p-4  bg-white rounded-lg shadow-xl dark:bg-gray-800 border-b-4  elevation-6" style="border-color:#FFC300;">
    ...
    <div class="relative w-full mt-4 mb-4 max-w-full flex-grow flex-1 px-2 py-2" v-if="response && user.ext_link.oit_up">
        <div class="grid gap-6 mb-8 md:grid-cols-2 xl:grid-cols-3">
          ...
          <div class="w-full   " v-for="year,is in upAll" :key="is">
            <div @click="$router.push(`/user/paperup?year=${year.id}`)" class="flex cursor-pointer	 items-center p-4  bg-white rounded-lg shadow-xl dark:bg-gray-800 border-b-4  elevation-6" style="border-color:#5e3c9d;">
  ...
  upAll:any = []
    ...
    public async getUP(){
      ...
      for (let i =0 ;i < yesrs.length;i++){
        let res = await Core.getHttp(`/api/ita/v1/rateresult/?rate__year=${this.years[i].id}&agency=${this.up}`)
        let rate = await Core.getHttp(`/api/ita/v2/rate/${this.years[i].id}/`)
