<template>
  <div class="md:pt-6 pb-32 pt-44 w-full" v-if="response"> 
    <v-toolbar>
      <h2 class="text-xl font-bold">ผลการประเมิน EIT  </h2> 
    ...
    <div class="mt-6">
      ...
      <div  v-for="(issue,i) in issues" class="m-3"
              :key="i" >
    <v-card  v-if="issue.name  !='ข้อเสนอแนะ'"  >
  ...
  async getIssue(){
    ...
    if(raw.length > 0){
      ...
      this.response = true;
             ...
             this.feedbacks  = await Core.getHttp(`/api/eit/v2/answersuggestioneit/?agency=${this.agencyData}&year=${this.yearFull.id}`)
