...
export default class TestDevClass extends Vue {
  ...
  async storeData(agency:any){
    ...
    let store = await Core.postHttp(`/api/report/v1/reportrawiit/`,{
      "year": this.yearData,
      "score": this.score,
  ...
  async getIssue(agency:any){
    ...
    this.issues = await CoreResult.getIssueIIT(this.year.id,agency.id)
  ...
  async getUserDone(agency:any){
    ...
    let user = await Core.getHttp(`/api/iit/v2/ansewer/user/?year=${this.year.id}&agency=${agency.id}`)
