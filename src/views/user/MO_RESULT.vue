...
<div class="relative md:pt-32 pb-32 pt-12 h-screen">
    ...
    <v-expansion-panels accordion>
        <v-expansion-panel v-for="(plan,i) in basePlans" :key="i">
            <v-expansion-panel-header>{{i+1}}. {{plan.name}}</v-expansion-panel-header>
...
export default {
    ...
    methods: {
        ...
        async getBasePlan() {
            this.basePlans = await Core.getHttp(`/api/moral_organization/baseplan/?year=${this.$route.query.year}&agency=${this.user.ext_link.agency}`)
        },
